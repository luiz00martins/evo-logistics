...
local bulk = require('/logos.logistics.storage.bulk')
local standard = require('/logos.logistics.storage.standard')
local crafting = require('/logos.logistics.storage.crafting')
local interface = require('/logos.logistics.storage.interface')
...
local function test_module()
	...
	peripheral = require('/logos.tests.mocks.peripheral')
	...
	local function reset()
		...
		main_cluster:registerInventory{inv_name = 'minecraft:barrel_0'}
		io_cluster:registerInventory{inv_name = 'minecraft:chest_0'}
		bulk_cluster:registerInventory{inv_name = 'techreborn:storage_unit_0'}
		crafting_cluster:registerInventory{inv_name = 'techreborn:auto_crafting_table_0'}
		crafting_cluster:registerInventory{inv_name = 'techreborn:assembly_machine_0'}
		interface_cluster:registerInventory{inv_name = 'minecraft:chest_1'}
	...
	io_cluster:refresh()
	assert_equals(transfer(io_cluster, bulk_cluster, nil, nil, 'minecraft:iron_ingot'), 1200)
	for slot=1,20 do
		peripheral.custom.produce_item('minecraft:chest_0', slot, 'minecraft:iron_ingot', 60)
	end
	io_cluster:refresh()
	assert_equals(transfer(io_cluster, bulk_cluster, nil, nil, 'minecraft:iron_ingot'), 976)
	...
	io_cluster:refresh()
	assert_equals(transfer(io_cluster, main_cluster, nil, nil), 12)
	...
	assert_equals(peripheral.inventories['minecraft:barrel_0'].items[1].count, 20)
	...
	output_cluster:registerInventory{inv_name = 'minecraft:barrel_1'}
	...
	input_cluster:registerInventory{inv_name = 'minecraft:barrel_1'}
